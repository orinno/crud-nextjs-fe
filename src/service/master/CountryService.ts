...
export async function getListUnit({
  ...
  try {
    const { data } = await ApiService.request(`/country`, "GET");
...
export async function createUnit(body: any): Promise<IResponseApi | null> {
  console.log(body);
  try {
    const { data } = await ApiService.request(`/country`, "POST", body);
...
export async function updateUnit(
  ...
  try {
    const { data } = await ApiService.request(`/country/${id}`, "PUT", body);
...
export async function deleteUnit(id: string): Promise<IResponseApi | null> {
  try {
    const { data } = await ApiService.request(`/country/${id}`, "DELETE");
